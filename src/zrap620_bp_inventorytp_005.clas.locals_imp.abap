...
CLASS lhc_inventory IMPLEMENTATION.
  ...
  METHOD CalculateInventoryID.
    "Ensure idempotence
    READ ENTITIES OF zrap620_r_inventorytp_005 IN LOCAL MODE
      ENTITY Inventory
        FIELDS ( InventoryID )
    ...
    "Get max travelID
    SELECT SINGLE FROM zrap620_inven005 FIELDS MAX( inventory_id ) INTO @DATA(max_inventory).

    "update involved instances
    MODIFY ENTITIES OF zrap620_r_inventorytp_005 IN LOCAL MODE
      ENTITY Inventory
  ...
  METHOD GetPrice.
    ...
    DATA proxy TYPE REF TO zrap620005co_epm_product_soap.
    ...
    LOOP AT inventories ASSIGNING FIELD-SYMBOL(<inventory>).
      ...
      TRY.
          ...
          ENDIF.
          IF proxy IS INITIAL.
            proxy = NEW zrap620005co_epm_product_soap(
                             destination = destination
                           ).
          ENDIF.

          DATA(request) = VALUE zrap620005req_msg_type( req_msg_type-product = <inventory>-ProductID ).
          proxy->get_price(
            EXPORTING
              input = request
            IMPORTING
        ...
        CATCH zrap620005cx_fault_msg_type INTO DATA(soap_exception).
